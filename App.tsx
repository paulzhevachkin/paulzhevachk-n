...
import { generateEditedImage, generateFilteredImage, generateAdjustedImage, generateAsciiArt, generateUpscaledImage } from './services/geminiService';
...
type Tab = 'retouch' | 'adjust' | 'filters' | 'crop' | 'ascii' | 'upscale';
...
const App: React.FC = () => {
  ...
  const handleApplyAsciiArt = useCallback(async (stylePrompt: string) => {
    if (!currentImage) {
        setError('No image loaded to apply ASCII art to.');
        return;
    }
    ...
    try {
        const asciiArtUrl = await generateAsciiArt(currentImage, stylePrompt);
        const newImageFile = dataURLtoFile(asciiArtUrl, `ascii-${Date.now()}.png`);
        addImageToHistory(newImageFile);
    } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
        setError(`Failed to generate ASCII art. ${errorMessage}`);
        console.error(err);
    } finally {
        setIsLoading(false);
    }
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-2 backdrop-blur-sm">
            {(['retouch', 'crop', 'adjust', 'filters', 'ascii', 'upscale'] as Tab[]).map(tab => (
                 <button
                    key={tab}
        ...
        <div className="w-full">
            ...
            {activeTab === 'ascii' && <AsciiPanel onApplyAsciiArt={handleApplyAsciiArt} isLoading={isLoading} />}
